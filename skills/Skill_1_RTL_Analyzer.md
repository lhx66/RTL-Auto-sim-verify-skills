...
## Rules (核心规则)
- **绝对禁止 VCD**：不要在 Testbench 中生成任何 `$dumpfile` 或 `$dumpvars` 语句。我们的系统通过分析文本日志进行 Debug。
...
> “请问上述识别出的层级结构与设计意图是否准确？由于我们将采用日志与断言驱动的验证方式，请您补充说明：**在测试中，您最关心哪些关键信号的跳变或哪些特定场景下的输出？** 我将针对您的描述，重新调整分析或设计专属的打印捕获逻辑。直至您完全认可后，我们再进入下一步。”
