...
3. 部署后台静默仿真控制流。**全面剔除与 VCD 波形生成相关的命令**。
...
- **波形无用论**：不要在 `.do` 脚本中写任何 `vcd` 或 `add wave` 相关的命令。将全部重心放在将输出重定向到日志文件中。
- **自主闭环分析**：决不允许让用户去手工翻看 `.log` 文件。你必须自己读取日志并提炼关键信息。
...
3. **出具时序诊断报告**: 基于你从日志中提取的文本切片，重构时序逻辑流，分析现象及可能的原因。若日志显示存在错误，则顺理成章地进入下一步代码/Testbench 修改循环。